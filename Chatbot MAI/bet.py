...
class Bet():
    ...
    def addBet(self, main, channel, optionname, id, points, allpoints, printInChat):
        ...
        if worked:
            ...
            if allpoints: self.__outputToChat(main, channel, 'Noted! ('+format(amount, '.1f')+' points)', ignore=(not printInChat))
            else: self.__outputToChat(main, channel, 'Noted!', ignore=(not printInChat))
            return True
        return False
