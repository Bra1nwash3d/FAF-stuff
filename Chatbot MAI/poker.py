...
class Poker(threading.Thread):
    def __init__(self, bot, callbackf, chatpointsObj, chateventsObj, maxpoints):
    ...
    def __onGameEnd(self, channel):
        ...
        if len(self.playerOrder) == 1:
            ...
            self.__outputToChat(channel, "Poker game over! {name} wins {stake} points!".format(**{
                'name' : winnername,
                'stake' : str(stake),
            }))
        else:
            ...
            if len(winners) == 1:
                # single winner
                self.__outputToChat(channel, "Poker game over! {name} wins {stake} points with {type}! ({cards} --- {midcards})!".format(**stringFormat))
            else:
                # multiple winner :O
                stake = int(stake / len(winners))
                stringFormat['name'] = ", ".join(winners)
                stringFormat['stake'] = str(stake)
                self.__outputToChat(channel, "Poker game over! {name} win {stake} points each with {type}!".format(**stringFormat))
        # transfer points
        losersDict = {}
        winnersDict = {}
        for name in self.players.keys():
    ...
    def beginNewRound(self, channel):
        if self.knownCards >= len(self.midCards):
            self.__onGameEnd(channel)
            return
        if len(self.playerOrder) <= 1:
            self.__onGameEnd(channel)
            return
        ...
        self.__outputToChat(channel, "Beginning new round! Known cards: [{cards}], Order is: [{order}]".format(**{
            "cards" : self.__readableCardList(self.midCards[:self.knownCards]),
            "order" : ", ".join(self.playerOrder),
        }))
        for name in self.playerOrder:
            self.__updatePlayer(channel, name, self.roundcosts[self.knownCards], wipeRoundPoints=True)

    def beginFirstRound(self, channel, name):
        if (self.knownCards < 3) and (name in self.playerOrder):
            self.beginNewRound(channel)

    def __isPlayersTurn(self, channel, name):
        if not self.gameIsRunning:
            return False
        nextPlayer = self.playerOrder[self.nextPlayer]
        if self.acceptingNewPlayers:
            self.__outputToChat(channel, "Still accepting new players! Patience please!")
            return False
        if not self.gameIsRunning:
            self.__outputToChat(name, "The game is over!")
            return False
        if not (name == nextPlayer):
            self.__outputToChat(channel, "It's {}'s turn!".format(nextPlayer))
            return False
        return True

    def __updatePlayer(self, channel, name, points, wipeRoundPoints=False):
        if not self.players.get(name, False):
            self.__outputToChat(channel, "You aren't participating, {}!".format(name))
            return False, 0
    ...
    def __informNext(self, channel, name, playerDropped=False):
        if len(self.playerOrder) == 1:
            self.__onGameEnd(channel)
            return
        ...
        if self.gameIsRunning:
            self.__outputToChat(self.playerOrder[self.nextPlayer], "Your turn!")
    ...
    def signup(self, channel, name):
        ...
        if not self.__reservePlayerPoints(name):
            self.__outputToChat(channel, "Sorry {name}, you require at least {points} points to join this game!".format(**{
                'name' : name,
        ...
        if worked:
            ...
            self.__outputToChat(channel, "No cards left!")
        if len(self.playerOrder) == 1:
            self.__outputToChat(channel, "A new poker game! Use '!cpoker signup' to join, and '!cpoker start' to start it!")
        self.lock.release()
        return True

    #@asyncio.coroutine
    def fold(self, channel, name):
        self.lock.acquire()
        if not self.__isPlayersTurn(channel, name):
            self.lock.release()
            return False
        self.__updatePlayer(channel, name, 0, wipeRoundPoints=True)
        self.playerOrder.pop(self.nextPlayer)
        self.__informNext(channel, name, playerDropped=True)
        self.lock.release()
        return True

    #@asyncio.coroutine
    def call(self, channel, name):
        self.lock.acquire()
        if not self.__isPlayersTurn(channel, name):
            self.lock.release()
            return False
        ...
        self.__updatePlayer(channel, name, missingPoints, wipeRoundPoints=False)
        self.__informNext(channel, name)
        self.lock.release()
        return True

    #@asyncio.coroutine
    def raise_(self, channel, name, points):
        self.lock.acquire()
        if not self.__isPlayersTurn(channel, name):
            self.lock.release()
            return False
        missingPoints = (self.currentStake - self.players[name]['roundpoints']) + points
        worked, paidPoints = self.__updatePlayer(channel, name, missingPoints, wipeRoundPoints=False)
        if paidPoints > 0:
            self.continuousCalls = 1
...
game = 7
if game == 0:
    poker = Poker(None, None, None, 60)
    poker.signup("#shadows", "Washy")
    poker.signup("#shadows", "guy2")
    poker.signup("#shadows", "guy3")
    poker.signup("#shadows", "guy4")
    print('-'*80)
    poker.beginNewRound("#shadows")
    poker.fold("#shadows", "Washy")
    poker.call("#shadows", "guy2")
    poker.call("#shadows", "guy3")
    poker.raise_("#shadows", "guy4", 6)
    poker.call("#shadows", "guy2")
    poker.raise_("#shadows", "guy3", 8)
    poker.call("#shadows", "guy4")
    poker.raise_("#shadows", "guy2", 2)
    poker.call("#shadows", "guy3")
    poker.call("#shadows", "guy4")

    print('-'*80)
    poker.call("#shadows", "guy2")
    poker.raise_("#shadows", "guy3", 5)
    poker.fold("#shadows", "guy4")
    poker.call("#shadows", "guy2")

    print('-'*80)
    poker.call("#shadows", "guy3")
    poker.call("#shadows", "guy2")
    #poker.fold("#shadows", "guy2")
elif game == 1:
    poker = Poker(None, None, None, 60)
    poker.signup("#shadows", "Washy")
    poker.signup("#shadows", "guy2")
    ...
    poker.signup("#shadows", "guy4")
    print('-'*80)
    poker.beginNewRound("#shadows") # TODO time?
    poker.call("#shadows", "Washy")
    poker.call("#shadows", "guy2")
    poker.call("#shadows", "guy3")
    poker.call("#shadows", "guy4")
    poker.call("#shadows", "Washy")
    ...
    print('-'*80)
    poker.beginNewRound("#shadows") # TODO time?
    poker.raise_("#shadows", "Washy", 100)
    poker.call("#shadows", "guy2")
    poker.call("#shadows", "guy3")
    poker.call("#shadows", "guy4")
    poker.call("#shadows", "Washy")
